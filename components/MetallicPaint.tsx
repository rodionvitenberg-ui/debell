...
// ОПТИМИЗАЦИЯ 1: mediump вместо highp. Существенно быстрее на мобилках.
...
function processImage(img: HTMLImageElement): ImageData {
  // ОПТИМИЗАЦИЯ 2: Радикально снижаем размер обрабатываемой картинки.
  // Было 1000, стало 512. Площадь меньше в 4 раза.
  const MAX_SIZE = 512;
  const MIN_SIZE = 256;
  ...
  // ОПТИМИЗАЦИЯ 3: Снижаем итерации симуляции на CPU с 200 до 30.
  // Это ускоряет "холодный старт" компонента в ~7 раз.
  const ITERATIONS = 30;
  const C = 0.01;
  const omega = 1.85;
...
export default function MetallicPaint({
  ...
  const initGL = useCallback(() => {
    ...
    // ОПТИМИЗАЦИЯ 4: low-power режим для GPU
    const gl = canvas.getContext('webgl2', { 
        antialias: false, // Отключаем сглаживание (оно дорогое)
        alpha: true,
  ...
  useEffect(() => {
    ...
    // ОПТИМИЗАЦИЯ 5: Жесткое ограничение разрешения.
    // Вместо умножения на devicePixelRatio (который может быть 3.0),
    // берем минимум между реальной шириной и 600px.
    // 600px более чем достаточно для "размытой" абстракции.
    const maxSide = 600; 
    const side = Math.min(maxSide, window.innerWidth);
  ...
  useEffect(() => {
    ...
    // Используем ImageBitmap если поддерживается (он быстрее), но для надежности оставим Image.
    // Важно, что processImage теперь намного быстрее.
    const img = new Image();
    img.crossOrigin = 'anonymous';
    img.onload = () => {
      // Это тяжелая операция, но теперь она в 10 раз легче
  ...
  // Остальной код эффектов без изменений...
  useEffect(() => {
    const gl = glRef.current;
